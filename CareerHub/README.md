...
The dashboard route `/dashboard/resume-analyzer` now supports a weighted ATS backend (FastAPI).
...
3.  **Set Gemini key (optional, improves insights quality)**
    Create a root `.env` (see `.env.example`) and add:
    ```env
    GEMINI_API_KEY=your_gemini_api_key_here
    ```
    You can also keep `VITE_GEMINI_API_KEY` there for frontend Gemini features.

4.  **Run ATS backend (FastAPI, recommended)**
    ```bash
    python -m uvicorn backend.fastapi_app:app --reload --port 8000
    ```

    **Alternative (legacy Flask backend)**
    ```bash
...
Default API endpoint used by the page: `POST http://localhost:8000/analyze`.
Fallback endpoint (legacy): `POST http://localhost:5000/analyze`.

### JD-Based Scoring Behavior

- Paste a target job description in the Resume Analyzer page before upload.
- Weighted scoring (FastAPI): Skills 40%, Projects 30%, Experience 20%, Education 10%.
- Backend extracts resume sections and scores each section against JD requirements.
- Response includes overall ATS score, section-wise scores, and missing skills.
